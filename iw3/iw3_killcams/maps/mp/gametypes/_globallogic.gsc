...
endGame( winner, endReasonText )
{
    ...
    if ( (level.roundLimit > 1 || (!level.roundLimit && level.scoreLimit != 1)) && !level.forcedEnd )
    {
		...
		if ( !hitRoundLimit() && !hitScoreLimit() )
		{
			if ( level.players.size > 0 && canStartFK( true ) )
			{
	...
	if ( level.players.size > 0 && canStartFK( false ) )
	{
		...
		if ( canStartFK( false ) )
		{
			...
			finalKillcamStarted = level.fk;
		}
	}

	if ( finalKillcamStarted )
		level waittill( "end_killcam" );
	else
...
//	self maps\mp\gametypes\_weapons::updateWeaponUsageStats();
	...
	onPlayerKilledHook(eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration);
