...
endGame( winner, endReasonText )
{
    ...
    if ( (level.roundLimit > 1 || (!level.roundLimit && level.scoreLimit != 1)) && !level.forcedEnd )
    {
		...
		if ( level.players.size > 0 )
		{
			...
			scripts\_finalkillcam::startFK( winner, true );
		}
        ...
        if ( !hitRoundLimit() && !hitScoreLimit() )
        {
			scripts\_finalkillcam::resetFinalKillcam();
	...
	if ( level.players.size > 0 && scripts\_finalkillcam::canStartFK( false ) )
	{
		wait 9;
		level.killcam_style = 0;
		scripts\_finalkillcam::startFK( winner, false );
	}
...
//	self maps\mp\gametypes\_weapons::updateWeaponUsageStats();
	...
	scripts\_finalkillcam::onPlayerKilled(eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration);
